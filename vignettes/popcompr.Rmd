---
title: "Comparing high resolution population datasets"
...
The package `popcompr` is an R package to make comparing different high resolution population datasets for humanitarian and research purposes. It is under active development and has not been released, but the code is available through a GPL3 license.

## An example comparing 2019 population estimates for Lesotho from World Pop & Facebook/CIESIN
Included in the package is a minimal working example for the country of Lesotho. 
...
### Compare populations at the pixel level
`lesotho_wp_2019` and `lesotho_fb_2019` are included in the package.
...
You can use your own raster and shapefile inputs, they just need to be in the same coordinate system/projection as each other! When using the shapefiles from geoBoundaries, input rasters must be in WGS84 (lat/long) coordinate system.
